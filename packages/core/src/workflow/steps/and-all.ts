...
>({ steps, ...config }: WorkflowStepParallelAllConfig<STEPS>) {
  // MUST use for all return values or types will NOT propagate correctly
  function coerceResult(result: any): INFERRED_RESULT {
    return result as INFERRED_RESULT;
  }
  ...
  return {
    ...
    execute: async (context) => {
      ...
      //   const promises = steps.map((step) => matchStep(step).execute(context));
      //   return coerceResult(await Promise.all(promises));
      // }
      ...
      //   const finalResults = coerceResult(results);
      ...
      //   return coerceResult(finalResults);
      // } catch (error) {
